...
class Dataset:
    ...
    @classmethod
    def from_dir(from_dircls, dir_: ty.Union[Path, str]) -> 'Dataset':
        dir_ = Path(dir_)
        ...
        return Dataset(
            ...
            json.loads(Path(dir_ / 'info.json').read_text()),
            dir_,
        )
    ...
    def build_X(
        ...
        *,
        ...
        num_nan_policy: str,  # 控制数值特征中的缺失值处理策略
        cat_nan_policy: str,  # 控制数值特征中的缺失值处理策略
        cat_policy: str,  # 用于控制类别特征的编码策略。如果类别特征需要进行编码，那么可以选择使用序号编码、独热编码、计数编码等不同的编码方式。
        cat_min_frequency: float = 0.0,
        # 不为零则：训练集中每个类别出现的次数，如果次数小于 cat_min_frequency，则将该类别替换为一个特殊的标记，例如 ___rare___。这样，模型就不会过于关注那些出现次数很少的类别
        seed: int,
        ...
        save_result(result)
        result = tuple(None if x is None else lib.to_tensors(x) for x in result)
