...
class Dataset:
    ...
    def from_dir(cls, dir_: ty.Union[Path, str]) -> 'Dataset':
        ...
        def load(item) -> ArrayDict:
            return {
                ...
                #{x: ... for x in ['train', 'val', 'test']} 是一个字典推导式，它可以用一行代码创建一个字典，其中 x 是键，后面的表达式是值。
    ...
    # build_X函数的作用是根据给定的参数构建输入数据X。
    #
    # 首先，函数会检查是否存在缓存文件，并根据缓存文件的存在与否决定是否使用缓存。如果存在缓存文件且缓存文件存在，则直接加载缓存文件并返回结果。
    #
    # 接下来，函数会对输入数据进行处理。如果存在类别型数据self.C且cat_policy不为
    # 'drop'，则需要对类别型数据进行处理。首先，根据cat_min_frequency参数确定最小出现次数，将出现次数小于该阈值的类别替换为一个特殊的标记值。然后，使用OrdinalEncoder对类别型数据进行编码，将类别转换为整数。最后，将未知值替换为最大值加1。根据cat_policy的不同，可以选择将编码后的类别数据与数值型数据N进行拼接，或者只返回编码后的类别数据。
    #
    # 如果不存在类别型数据self.C或者cat_policy为
    # 'drop'，则需要对数值型数据self.N进行处理。首先，根据num_nan_policy参数确定处理缺失值的策略。如果策略为
    # 'mean'，则将缺失值替换为每列的均值。最后，根据normalization参数对数值型数据进行归一化处理。
    #
    # 最后，将处理后的数据保存到缓存文件中，并返回结果。
    #
    # 总之，build_X函数根据给定的参数构建输入数据X，包括对类别型数据的编码和处理缺失值，以及对数值型数据的处理和归一化。
    def build_X(
        self,
        *,
        ...
        if cat_policy == 'indices':
            ...
            result = C if N is None else {x: np.hstack((N[x], C[x])) for x in N}
        else:            util.raise_unknown('categorical policy', cat_policy)
        save_result(result)
